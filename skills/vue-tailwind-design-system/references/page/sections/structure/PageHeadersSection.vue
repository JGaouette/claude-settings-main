...
import { Bell, Ellipsis, Search } from "lucide-vue-next";
...
<div class="border-b border-hairline pb-6">
  <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    <div>
      <h1>Projects</h1>
      <p class="mt-1">Everything your team is working on.</p>
    </div>

    <!-- Right-side slot — buttons, icons, search, links -->
    <div class="flex items-center gap-2">
      <button type="button" class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-hairline text-ink-body hover:bg-surface" aria-label="Notifications">
        <Bell class="h-4 w-4" />
      </button>
    ...
    <div class="flex items-center gap-2">
      <DropdownMenu>
        <DropdownMenuTrigger as-child>
          ...
          >
            <Ellipsis class="h-6 w-6" :stroke-width="1.5" />
          </button>
        </DropdownMenuTrigger>
...
<div>
  <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between border-b border-hairline pb-6">
    <div>
      <h1>Acme Inc.</h1>
      <p class="mt-1">Workspace settings and billing.</p>
    </div>
    <div class="flex items-center gap-2">
      <Button>Save</Button>
    </div>
  ...
  <nav class="flex items-end justify-between gap-4 border-b border-hairline">
    ...
    <div class="flex items-center gap-3 pb-2">
      <a href="#" class="text-sm text-ink-muted no-underline hover:text-ink-display">View audit log</a>
    </div>
  </nav>
</div>`;
</script>
...
<template>
  <SectionShell id="page-headers" title="Page headers" :code="code">
    <template #description>
      The titled banner at the top of a page's main content area. Holds the page
      title, optional supporting copy, and a flexible right-side slot for
      actions. Optionally appends sub-navigation tabs below the title row to
      produce a two-line header.
    </template>
    <template #whenToUse>
      <ul>
    ...
    <template #preview>
      <div class="space-y-10">
        ...
        <div>
          ...
          <div class="border-b border-hairline pb-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h1>Members</h1>
                <p class="mt-1">Manage who has access to this workspace.</p>
              </div>
              <div class="flex items-center gap-2">
                <div class="relative">
                  <Search class="pointer-events-none absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-muted" />
                  <Input class="pl-8" placeholder="Search members" />
                </div>
        ...
        <div>
          <p class="mb-2 text-xs uppercase tracking-wider text-ink-muted">
            Single-item view — title with settings dropdown (the standard layout
            for viewing one item from a collection)
          </p>
          <div class="border-b border-hairline pb-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div class="min-w-0 flex-1">
        ...
        <div>
          ...
          <div>
            ...
            <nav class="flex items-end justify-between gap-4 border-b border-hairline">
              ...
              <div class="flex items-center gap-3 pb-2">
                <span class="text-sm text-ink-muted">View audit log</span>
              </div>
            </nav>
          </div>
        </div>
    ...
    <template #options>
      <ul class="list-disc pl-5">
        <li>
          <strong>Right-side slot</strong>: a flex row that accepts any mix of
          buttons, icon buttons, links, badges, or search inputs. Place primary
          CTA at the far right; secondary actions to its left.
        </li>
        <li><strong>With breadcrumb</strong>: prepend a small breadcrumb row above the title.</li>
        <li>
          <strong>With tabs</strong>: append the Sub navigation pattern below the
          title row's <code>border-b</code> — the tabs row gets its own
          <code>border-b</code>, producing two horizontal lines with the active
          tab's underline merging into the lower one.
        </li>
        <li>
          <strong>Single-item view</strong>: when viewing one item from a
          collection, the right slot should hold our standard settings dropdown —
          see <a href="#dropdown-menu">Dropdown menu</a>. Use <code>h-6 w-6</code>
          on the <code>Ellipsis</code> here (one size larger than in listing rows)
          to match the page header's heavier visual weight.
        </li>
      </ul>
    </template>
  </SectionShell>
