...
const code = `<article class="body-content">
...
<template>
  <SectionShell id="body-content" title="Body content" :code="code">
    <template #description>
      ...
      (<code>space-y-6</code>) and breathing room above sub-headings. We do
      <em>not</em> use Tailwind's typography plugin.
    </template>
    <template #whenToUse>
      <ul>
    ...
    <template #preview>
      <article class="body-content text-ink-body">
        <h2>Section heading</h2>
        <p>
          Lead paragraph introducing the section. The body font and base type
          sizes come from the global stylesheet.
        </p>
        <p>
          A second paragraph shows the consistent vertical rhythm given by
          <code>.body-content &gt; * + *</code> with
          <code>margin-top: 1.5rem</code>.
        </p>
        <h3>Sub-section</h3>
        <p>
          Each <code>h2</code>, <code>h3</code>, or <code>h4</code> inside
          <code>.body-content</code> gets <code>pt-4</code> unless it's the first
          child.
        </p>
        <ul>
    ...
    <template #options>
      <ul class="list-disc pl-5">
        <li>
          <code>.body-content</code> applies vertical spacing only — no font or
          color overrides. Type comes from base styles.
        </li>
        <li>
          Custom widths are fine: pair with <code>max-w-prose</code> or your own
          constraint.
        </li>
      </ul>
    </template>
