...
<template>
  <SectionShell id="callout" title="Callout" :code="code">
    <template #description>
      A simple bordered container for emphasizing a chunk of content inside a
      longer page. Apply <code>.callout</code> to any element to get a hairline
      border, rounded corners, and comfortable padding.
    </template>
    <template #whenToUse>
      <ul>
    ...
    <template #preview>
      <div class="callout">
        <p class="text-ink-body">
          Heads up — this is a callout. Wrap any content that needs a visual
          container (warning, tip, summary, side note) in <code>.callout</code>.
        </p>
      </div>
    </template>
    <template #options>
      <ul class="list-disc pl-5">
        <li>
          ...
          rounded corners, padding. No background or color tone — pair with
          <code>bg-surface</code> or <code>bg-accent-faded</code> if you want
          emphasis.
        </li>
        <li>
          Compose with text utilities (<code>text-ink-display</code>,
          <code>text-ink-muted</code>) and inline icons for richer variants
