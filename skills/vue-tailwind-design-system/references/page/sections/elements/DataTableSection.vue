...
<template>
  <SectionShell id="data-table" title="Data table" :code="code">
    <template #description>
      A vertical key/value table for displaying record details. Each row is a
      horizontal stripe with a title on the left and a value on the right. Rows
      stack on mobile; from <code>md</code>, the title takes 1 column and the
      value spans 3 (out of 4); from <code>xl</code>, the value spans 4 (out of
      5) so wider screens give the value more room while the title stays the
      same size.
    </template>
    <template #whenToUse>
      <ul>
    ...
    <template #preview>
      <DataTable>
        ...
        <DataRow title="Description">
          ...
          breaking the visual rhythm of the table. On wider screens the value
          takes more room so long copy doesn't have to wrap as aggressively.
        </DataRow>
        <DataRow title="Notes">
          Drop any node in here — text, a Badge, a list, a small grid of nested
          values.
        </DataRow>
        <DataRow title="Updated">May 16, 2026 at 1:27pm</DataRow>
      </DataTable>
    ...
    <template #options>
      <ul class="list-disc pl-5">
        <li>
          <code>&lt;DataRow title&gt;</code> accepts a short label string; use
          the <code>#title</code> slot for a richer title (icon or badge).
        </li>
        <li>
        ...
        <li>
          ...
          <code>md</code> — title sits above value, divider rhythm preserved.
          From <code>md</code> (768px), 4 columns with title
          <code>md:col-span-1</code> and value <code>md:col-span-3</code>. From
          <code>xl</code> (1280px), 5 columns with value
          <code>xl:col-span-4</code>.
        </li>
        <li>
          Wrap the table in a card (<code>rounded-lg border border-hairline</code>)
          if it needs to feel like its own panel.
        </li>
      </ul>
    </template>
  </SectionShell>
