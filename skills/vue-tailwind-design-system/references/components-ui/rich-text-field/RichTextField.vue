...
// Two-way markdown value. Its value at mount time seeds the editor; later
// edits flow back out. The editor is uncontrolled after creation, matching
// the underlying Crepe behavior.
...
onBeforeUnmount(() => {
  if (onKeyDown && root.value) root.value.removeEventListener("keydown", onKeyDown);
  crepe?.destroy();
  crepe = null;
});
</script>
...
<template>
  <div
    ...
    :class="
      cn(
        'vue-rich-text rounded-md border border-hairline bg-page text-ink-body',
